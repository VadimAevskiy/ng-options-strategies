...
This report develops two options trading strategies on Chicago Mercantile Exchange (CME) Henry Hub Natural Gas (NG) futures. The dataset covers <<N_DAYS>> trading days and <<N_OPTIONS>> option observations. The pipeline is: Exploratory Data Analysis (EDA) $\to$ signal construction $\to$ walk-forward backtest $\to$ stability verification.
...
\midrule
Total P\&L & <<OVERLAY_PNL>> & <<BH_PNL>> & <<ALPHA_PNL>> \\
Sharpe ratio & <<OVERLAY_SR>> & <<BH_SR>> & <<ALPHA_SR>> \\
Calmar ratio & <<OVERLAY_CALMAR>> & <<BH_CALMAR>> & <<ALPHA_CALMAR>> \\
Win rate & <<OVERLAY_WR>>\% & <<BH_WR>>\% & <<ALPHA_WR>>\% \\
Profit factor & <<OVERLAY_PF>> & <<BH_PF>> & <<ALPHA_PF>> \\
Trades & -- & -- & <<N_TRADES>> \\
Trade VaR (95\%) & $<<OVERLAY_VAR>>$ & $<<BH_VAR>>$ & $<<ALPHA_VAR>>$ \\
\bottomrule
\end{tabular}
\end{table}

The strangle alpha adds +590 to the B\&H base (+165 $\to$ +755). The overlay Sharpe (2.00) is 3.2$\times$ the B\&H Sharpe (0.62). The alpha is statistically significant ($t = <<T_STAT>>$, $p <<P_VAL_STR>>$). Bootstrap test: $P(\text{overlay} > \text{B\&H}) = 98\%$. All three years are profitable: 2020 (+125), 2021 (+143), 2022 (+322).
...
The Jarque-Bera test rejects normality ($\chi^2 = <<JB_STAT>>$, $p < 0.001$). Excess kurtosis is <<KURTOSIS>>. The Kolmogorov-Smirnov test cannot reject a Student-$t$ distribution ($p = <<KS_T_PVAL>>$) but rejects normal at the 15\% level.
...
\midrule
Winter (Nov--Feb) & $<<VRP_W>>$ & <<VRP_W_POS>>\% \\
Spring (Mar--May) & $<<VRP_SP>>$ & <<VRP_SP_POS>>\% \\
Summer (Jun--Aug) & $<<VRP_SU>>$ & <<VRP_SU_POS>>\% \\
Fall (Sep--Oct) & $<<VRP_F>>$ & <<VRP_F_POS>>\% \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Low-vol ($s=0$) & <<R_LOW_VOL>>\% & <<R_LOW_DUR>> days & <<R_LOW_FREQ>>\% \\
High-vol ($s=1$) & <<R_HIGH_VOL>>\% & <<R_HIGH_DUR>> days & <<R_HIGH_FREQ>>\% \\
\bottomrule
\end{tabular}
\end{table}

Transition probabilities: $p_{00} = <<P00>>$, $p_{11} = <<P11>>$. The high-vol regime is short-lived but intense.
...
\midrule
Total P\&L & <<S2_BH_PNL>> & <<S2_BS_PNL>> & <<S2_SV_PNL>> \\
Sharpe & <<S2_BH_SR>> & <<S2_BS_SR>> & <<S2_SV_SR>> \\
Calmar & <<S2_BH_CAL>> & <<S2_BS_CAL>> & <<S2_SV_CAL>> \\
Max Drawdown & $<<S2_BH_DD>>$ & $<<S2_BS_DD>>$ & $<<S2_SV_DD>>$ \\
Ann.\ Vol & <<S2_BH_VOL>> & <<S2_BS_VOL>> & <<S2_SV_VOL>> \\
Time above B\&H & --- & <<S2_BS_PAB>>\% & <<S2_SV_PAB>>\% \\
\bottomrule
\end{tabular}
\end{table}

The vol-targeted overlay improves Sharpe by 2.4$\times$ (<<S2_BH_SR>> $\to$ <<S2_SV_SR>>) and reduces max drawdown by 60\% ($<<S2_BH_DD>> \to <<S2_SV_DD>>$), while \emph{increasing} total P\&L from <<S2_BH_PNL>> to <<S2_SV_PNL>>. Critically, the strategy's cumulative P\&L line sits \emph{above} B\&H for <<S2_SV_PAB>>\% of trading days --- it does not merely ``catch up'' during crashes but systematically outperforms throughout the entire backtest period. The leverage from vol-targeting in calm markets generates the extra return that funds the crisis protection.
...
Max drawdown & $<<OV_DD>>$ & $<<BH_DD>>$ & $<<A_DD>>$ \\
Information ratio & -- & -- & <<INFO_R>> \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
SHORT (vol reversion) & <<N_SHORT>> & <<SHORT_WR>>\% & <<SHORT_AVG>> & <<SHORT_PNL>> \\
LONG (summer injection) & <<N_LONG>> & <<LONG_WR>>\% & <<LONG_AVG>> & <<LONG_PNL>> \\
\midrule
Combined & <<N_TRADES>> & <<ALL_WR>>\% & <<ALL_AVG>> & <<ALPHA_PNL>> \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Above median & <<GH_N>> & <<GH_WR>>\% & <<GH_AVG>> & <<GH_PNL>> \\
Below median & <<GL_N>> & <<GL_WR>>\% & $<<GL_AVG>>$ & $<<GL_PNL>>$ \\
\bottomrule
\end{tabular}
\end{table}
